			...
			if ( r.isUserDefined ) {
				...
				var k = function() {
					if (args.length == 0) {
						...
						var arg = args.shift();
						if (arg.constructor === RemoteObject) {
	...
	RemoteObjectManager.prototype.receiveMessage = function(data) {
		...
		try {
			...
			case 'call':
				...
				inner.apply_cps(reciever,
						this.getRemoteObject(data.thisObj),
						args,
						function(result) { returnResult( that.exportObject(result) ); });
				break;
			case 'set':
				returnResult( this.exportObject(reciever[data.prop] = this.getRemoteObject(data.value)) );
				break;
			case 'get':
				returnResult( this.exportObject(reciever[data.prop]) );
				break;
//			case 'invoke':
//				break;
			case 'each':
...
//		return this.exportedObjects[objData.objectId];
	...
	RemoteObjectManager.prototype.getObjectId = function(object) {
		...
		console.log("getObjectId:"+objectId);
		return objectId;
	};
...
//		}
	...
	RemoteObject.prototype.set_cps = function(prop, value, k) {
		this.sendMessage({type:'set', value:this.manager.exportObject(value), prop:prop}, k);
	};
...
//	};
	...
	RemoteObject.prototype.apply_cps = function(thisObj, args, callback) {
		var args_ = [];
		for (var i = 0; i < args.length; i++) {
			args_.push( this.manager.exportObject(args[i]) );
		}
		this.sendMessage({type:'call', args:args_, thisObj:this.manager.exportObject(thisObj) }, callback);
	};
})();
