...
(function() {
	...
	Function.prototype.apply_cps = function(thisObj, args, callback) {
		if (this.userDefined) {
			args.push(callback);
			...
			var k = function() {
				if (args.length == 0) {
					callback(this.apply(thisObj, args_));
				} else {
					var arg = args.shift();
			...
			try {
				...
				case 'call':
					...
					for (var i = 0; i < data.args.length; i++)
						args.push( getRemoteObject(data.args[i]) );
					reciever.apply_cps(getRemoteObject(data.thisObj), args, function(result) { returnResult( that.exportObject(result) ); });
					break;
				case 'set':
					returnResult( that.exportObject(reciever[data.prop] = getRemoteObject(data.value)) );
					break;
				case 'get':
	...
	RemoteFunction.prototype.apply_cps = function(thisObj, args, callback) {
		...
		this.sendMessage({type:'call', args:args, thisObj:exportObject(thisObj) }, callback);
	};
})();
